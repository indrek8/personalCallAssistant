...
| D21 | **Re-analyze reuses `run_post_analysis`** (state-agnostic) and **restores the prior status on failure** (was hardcoded `ending`) | A failed Re-analyze of a `completed` session keeps it `completed` with its existing analysis intact | **M5** |
| D22 | **Post screen has an entry `postMode` (`fresh`/`reanalyze`/`resume`)** — `resume` loads the draft without a Sonnet call | Idempotent Post; powers recover-into-review without re-billing | **M5** |
| D23 | **Recover-into-review:** a crashed `reviewing` session **stays `reviewing`** (keeps its draft) and reopens via an actionable toast (supersedes D20's reviewing clause) | The user resumes a half-reviewed session instead of it silently completing | **M5** |
| D24 | **Global `labels.json` registry**; sessions keep embedded `LabelRef` snapshots, the dashboard resolves id→name/color from the registry | Rename/recolor reflects everywhere; delete leaves self-describing orphans (registry-only) | **M5** |
| D25 | **EXC-CORRUPT is a visible row** — `SessionMeta.unreadable`; `list_sessions` surfaces a placeholder (id = dir name) instead of skipping | "⚠ Unreadable" + Reveal in Finder; the list never crashes on one bad row | **M5** |
| D26 | **`set_capture_device` folded into `save_settings`** (no redundant command); **`reveal_in_finder`** via the opener plugin | Avoids a duplicate path; reconciles the §7 contract to reality | **M5** |

...
> **Status:** **M4 complete** (branch `feat/m4-post-analysis`) — post-analysis: End → Sonnet structured extraction → merge with live/saved commitments → review/edit → Save & Close, with the `ending → analyzing → reviewing → completed` state machine and EXC-API-POST / EXC-EMPTY handling. 96 unit tests, clippy + svelte-check clean; the on-device run (a real call → review → save) is the remaining manual check. **M3 complete** (PRs #9–#12) — the live-AI layer: Claude client + macOS-Keychain keys, Haiku findings with F/C/S/Q toggles + cost meter, streamed Sonnet Ask-AI. **M2 complete** — the capture → live two-sided transcript engine (dual-stream → 16 kHz stereo WAV, VAD → Whisper, crash recovery, EXC-DEV-DROP). **M1/M0 complete** — app shell; all four spikes validated. Next: **M5 — Manage, Settings & Polish** (dashboard detail, labels, settings, error surfacing). See **[docs/build/milestones.md](docs/build/milestones.md)**.
